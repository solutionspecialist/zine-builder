...
def process_image(pil_img, scale_mode, target_w, target_h):
    ...
    Args:
        ...
        scale_mode: "fit" or "fill" scaling mode
        target_w: Target width in points
        target_h: Target height in points

    Returns:
        tuple: (processed_image, new_width, new_height)
    """
...
def create_zine_pdf(ready_pages, show_guides, project_title, margin_inches, gutter_px):
    """
    Args:
        ready_pages: Dict of page data with processed images
        show_guides: Whether to show fold/cut guides
        project_title: Title for the PDF metadata
        margin_inches: External margin in inches
        gutter_px: Internal gutter in pixels
    ...
    # Page layout mapping: {page_num: (column, row, rotation)}
    layout = {
        5: (0, 0, 180),
        4: (1, 0, 180),
        3: (2, 0, 180),
    ...
    # Spread links: {anchor_page: target_page}
    links = {1: 8, 2: 3, 4: 5, 6: 7}
    processed_targets = []

    # Process pages in imposition order
    ...
    # Add fold/cut guides if requested
    if show_guides:
        c.setStrokeColorRGB(0.7, 0.7, 0.7)
        c.setDash(2, 2)
        for i in range(1, 4):
            c.line(i * slot_w, 0, i * slot_w, height)
